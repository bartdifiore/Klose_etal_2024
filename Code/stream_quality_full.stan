...
parameters {
  ...
  real alpha_quality; // Intercept for stream quality model
...
transformed parameters {
  ...
  // Stream quality predicted by burn and drought
...
model {
  ...
  // Priors on stream quality model
  alpha_quality ~ normal(0, 2);
  beta_burned ~ normal(0, 1);
  beta_wet ~ normal(0, 1);
  ...
  sigma_canopy ~ exponential(0.5);
  sigma_q ~ exponential(0.5);
