...
parameters {
  ...
  real alpha_sq;      // Add back in the intercept
...
transformed parameters {
  ...
  stream_quality_hat = alpha_sq + beta_burned * burned + beta_wet * wet;
...
model {
  ...
  beta_conduct ~ normal(0, 2);
  beta_depth ~ exponential(1.8);    // Directional prior for identification
  beta_do ~ normal(0,2);       // Directional prior (higher DO = better quality)
  beta_thermal ~ normal(0, 2);     // Expect negative (higher thermal = worse quality)
  ...
  beta_trout_presence ~ normal(0, 2);  // Regularizing prior: centered at +1, allows negative
...
generated quantities {
  ...
  int trout_count_rep[N];
   ...
   // Latent stream quality means by burn × wet category
  real mu_unburned_dry;   // U–D (baseline)
  real mu_burned_dry;     // B–D
  real mu_unburned_wet;   // U–W
  real mu_burned_wet;     // B–W

  // Define group means
  mu_unburned_dry = alpha_sq;
  mu_burned_dry   = alpha_sq + beta_burned;
  mu_unburned_wet = alpha_sq + beta_wet;
  mu_burned_wet   = alpha_sq + beta_burned + beta_wet;
